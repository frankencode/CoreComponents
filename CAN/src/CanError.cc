...
static_assert(+CanError::Type::TransceiverStatus == CAN_ERR_TRX);
...
String str(CanError::Type type)
{
    ...
    if (type & CanError::Type::TransceiverStatus) parts << "CanError::Type::TransceiverStatus";
...
String CanError::toString() const
{
    Format f;
    f <<
        "{\n"
    ...
    if (type() == Type::LostArbitration) {
        f << "  \"bit-pos\": \"" << bitPos() << "\",\n";
    }

    if (controller() != Controller::Unspec) {
        f << "  \"controller\": \"" << controller() << "\",\n";
    }

    if (protocol() != Protocol::Unspec) {
        f << "  \"protocol\": \"" << protocol() << "\",\n";
    }
