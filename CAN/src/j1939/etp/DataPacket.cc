...
CanFrame DataPacket::produce(const Bytes &message, uint32_t batchOffset, uint8_t packetNumber, uint8_t src, uint8_t dst)
{
    const uint32_t offset = batchOffset + (packetNumber - 1) * 7;
