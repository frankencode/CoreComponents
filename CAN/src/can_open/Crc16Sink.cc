...
struct Crc16Sink::State final: public HashSink::State
{
    ...
    State(uint16_t seed = Crc16Sink::DefaultSeed):
        crc_{seed}
    {}
    ...
    void feed(const void *buf, int bufFill)
    {
        ...
        for (int i = 0; i < bufFill; ++i) {
            int k = ((crc_ & 0xFF00U) >> 8) ^ static_cast<uint16_t>(data[i]);
    ...
    {
        auto sum = String::allocate(Crc16Sink::HashSize);
        sum[0] = (crc_ >> 8) & 0xFFU;
        sum[1] = crc_ & 0xFFU;
        return sum;
    }
...
Crc16Sink::Crc16Sink(uint16_t seed):
    HashSink{new State{seed}}
{}

uint16_t Crc16Sink::sum() const
{
    return me().crc_;
}

const Crc16Sink::State &Crc16Sink::me() const
{
...
{
    Crc16Sink::State state;
    if (buffer) state.feed(buffer, strlen(buffer));
    return state.crc_;
}
