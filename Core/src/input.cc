...
List<String> scanCsv(const String &line)
{
    enum class Mode {
        OutsideText,
        InsideText,
    ...
    Mode mode = Mode::OutsideText;
    List<String> items;
    long j = 0, j0 = 0;

    for (; j < line.count(); ++j)
    {
        if (mode == Mode::OutsideText) {
            if (line.at(j) == ',') {
                items.append(line.copy(j0, j));
                j0 = j + 1;
            }
            else if (line.at(j) == '"') {
                mode = Mode::InsideText;
                j0 = j + 1;
            }
        }
        else if (mode == Mode::InsideText) {
            if (line.at(j) == '"' && (j == 0 || line.at(j - 1) != '\\')) {
                String item = line.copy(j0, j);
                item.replace("\\\"", "\"");
                items.append(item);
                mode = Mode::AfterText;
            }
        }
        else if (mode == Mode::AfterText) {
            if (line.at(j) == ',') {
                mode = Mode::OutsideText;
                j0 = j + 1;
            }
        }
    }

    if (mode == Mode::OutsideText && j0 < j) {
        items.append(line.copy(j0, j));
    }
