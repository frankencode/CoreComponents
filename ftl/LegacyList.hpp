/*
 * LegacyList.hpp -- double-linked list
 *
 * Copyright (c) 2007-2011, Frank Mertens
 *
 * See ../COPYING for the license.
 */
#ifndef FTL_LEGACYLIST_HPP
#define FTL_LEGACYLIST_HPP

#include "containers.hpp"
#include "Heap.hpp"
#include "LegacyListNode.hpp"
#include "LegacyListWalker.hpp"
...
template<class T>
class LegacyList:
	public Container< T, LegacyList<T> >,
	public Sequence< T, LegacyListWalker<T> >
{
public:
	typedef LegacyListWalker<T> Index;
	typedef GenericIterator< LegacyList<T> > Iterator;
	typedef T Item;
	
	LegacyList(int maxLength = intMax);
	~LegacyList();
	...
	LegacyList& push(Index& index, const T& item);
	LegacyList& pop(Index& index, T& item);
	void clear();
	...
	inline LegacyList& push(const T& item) { return push(end(), item); }
	inline LegacyList& pop(T& item) { return pop(first(), item); }
	...
	Ref<LegacyList, Owner> sort(int order = SortOrder::Ascending, bool unique = false) const;
	Ref<LegacyList, Owner> unique(int order = SortOrder::Ascending) const { return sort(order, true); }
	
	// return value to reference conversion wrappers
	inline LegacyList& push(Return<Index> ret, const T& item) {
		Index index = ret;
		return push(index, item);
	}
	inline LegacyList& pop(Return<Index> ret, T& item) {
		Index index = ret;
	...
	inline LegacyList& insert(Return<Index> ret, const T& item) {
		Index index = ret;
		return push(index, item);
	}
	inline LegacyList& remove(Return<Index> ret, T& item) {
		Index index = ret;
		return pop(index, item);
	}
...
template<class T>
LegacyList<T>::LegacyList(int maxLength)
	: firstNode_(0),
	  lastNode_(0),
	  length_(0),
...
template<class T>
LegacyList<T>::~LegacyList()
{
	clear();
}

template<class T>
LegacyList<T>& LegacyList<T>::push(Index& index, const T& item)
{
	check(index.list_ == this);
...
template<class T>
LegacyList<T>& LegacyList<T>::pop(Index& index, T& oldItem)
{
	check(index.list_ == this);
	check(index.valid());
...
template<class T>
typename LegacyList<T>::Index LegacyList<T>::find(const T& item, Index index) const
{
	while (index.valid()) {
...
template<class T>
int LegacyList<T>::replace(const T& oldItem, const T& newItem)
{
	Index index = first();
	int numReplaced = 0;
	while (index.valid()) {
...
template<class T>
Ref<LegacyList<T>, Owner> LegacyList<T>::sort(int order, bool unique) const
{
	Ref<LegacyList, Owner> result = new LegacyList;
