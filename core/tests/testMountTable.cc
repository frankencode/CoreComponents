...
int main(int argc, char *argv[])
{
    ...
    TestCase {
        ...
        []{
            ...
            MountTable table = MountTable::read(tableText);
            for (const auto &entry: table)
                fout() << entry.source() << " -> " << entry.target() << " (" << entry.type() << ": " << entry.options() << ")" << nl;

            CC_CHECK(table.count() == 48);
            CC_CHECK(table.at(table.count() - 1).source() == "gvfsd-fuse");
            CC_CHECK(table.at(table.count() - 1).target() == "/run/user/1001/gvfs");
            CC_CHECK(table.at(table.count() - 1).type() == "fuse.gvfsd-fuse");
            CC_CHECK(table.at(table.count() - 1).options() == "rw,nosuid,nodev,relatime,user_id=1001,group_id=1001");
        }
    };
