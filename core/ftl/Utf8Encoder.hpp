...
{
public:
	inline static O<Utf8Encoder> open(Stream *stream, int bufCapa = FTL_DEFAULT_BUF_CAPA) {
		return new Utf8Encoder(stream, bufCapa);
	}

	inline static O<Utf8Encoder> open(void *buf, int bufCapa) {
