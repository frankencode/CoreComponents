...
O<ByteArray> Base64::encode(const ByteArray *source)
{
	const char *alphabet =
	...
	O<ByteArray> sink = ByteArray::create(m);
	int l = 0;
...
O<ByteArray> Base64::decode(const ByteArray *source)
{
	...
	int n = 3 * (m / 4) - p;
	O<ByteArray> sink = ByteArray::create(n);
	int i = 0;
