...
{
public:
	inline static Ref<BitDecoder> open(Stream *stream, int bufCapa = FTL_DEFAULT_BUF_CAPA, int endian = FTL_DEFAULT_ENDIAN) {
		return new BitDecoder(stream, bufCapa, endian);
	}
	inline static Ref<BitDecoder> open(void *buf, int bufCapa, int endian = FTL_DEFAULT_ENDIAN) {
