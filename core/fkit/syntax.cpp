...
void DefinitionNode::LINK(bool optimize)
{
	...
	if (!LinkNode::rule_) {
		if (!LinkNode::ruleName_)
			FKIT_THROW(DebugException, "Missing entry rule declaration");
	...
	while (true) {
		...
		if (k == 2) {
			...
			if (!scope->definitionByName_->lookup(p0, p - p0 - k, &childScope))
				FKIT_THROW(DebugException, strcat("Undefined scope '", name, "' referenced"));
			scope = childScope;
			p0 = p;
			k = 0;
		}
...
{
	FKIT_THROW(DebugException, "Unhandled syntax error");
	return -1;
}
