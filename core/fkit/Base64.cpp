...
{
	FKIT_CHECK(source->size() % 4 == 0, EncodingException, "base64: encoded input needs to be a multiple of 4 bytes");
	...
	FKIT_CHECK((0 <= p) && (p <= 2), EncodingException, "base64: illegal padding");
	...
	for (int l = 0; l < m;) {
		...
		for (int k = 0; k < 4; ++k) {
			...
			else FKIT_CHECK(false, EncodingException, "base64: illegal character");
			bits |= ch;
			bits <<= 6;
		}
		bits >>= 6;
