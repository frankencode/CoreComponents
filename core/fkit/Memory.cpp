...
#define FKIT_MEM_GRANULARITY 16
#define FKIT_MEM_ALIGN(x) ((x) / FKIT_MEM_GRANULARITY + ((x) % FKIT_MEM_GRANULARITY > 0)) * FKIT_MEM_GRANULARITY
...
void *Memory::allocate(size_t size)
{
	...
	if (!bucket) {
		if (size > pageSize - FKIT_MEM_ALIGN(sizeof(BucketHeader))) {
			...
			*(uint32_t *)pageStart = pageCount;
			...
			bucket->bytesDirty_ = FKIT_MEM_ALIGN(sizeof(BucketHeader)) + size;
			bucket->objectCount_ = 1;
			bucket->open_ = true;
			allocator->bucket_ = bucket;
			return (void *)((char *)pageStart + FKIT_MEM_ALIGN(sizeof(BucketHeader)));
		}
	}
	else {
		bucket->acquire();
