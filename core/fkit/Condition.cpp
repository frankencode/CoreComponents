...
Condition::Condition()
{
	...
	if (ret != 0)
		FKIT_PTHREAD_EXCEPTION("pthread_cond_init", ret);
}
...
Condition::~Condition()
{
	...
	if (ret != 0)
		FKIT_PTHREAD_EXCEPTION("pthread_cond_destroy", ret);
}
...
void Condition::wait(Mutex *mutex)
{
	...
	if (ret != 0)
		FKIT_PTHREAD_EXCEPTION("pthread_cond_wait", ret);
}
...
bool Condition::waitUntil(double timeout, Mutex *mutex)
{
	...
	if (ret != 0) {
		if (ret == ETIMEDOUT)
			...
			FKIT_PTHREAD_EXCEPTION("pthread_cond_timedwait", ret);
	}
	return success;
}
...
void Condition::signal()
{
	...
	if (ret != 0)
		FKIT_PTHREAD_EXCEPTION("pthread_cond_signal", ret);
}
...
void Condition::broadcast()
{
	...
	if (ret != 0)
		FKIT_PTHREAD_EXCEPTION("pthread_cond_broadcast", ret);
}
