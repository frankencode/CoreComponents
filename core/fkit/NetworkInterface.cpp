...
Ref<NetworkInterfaceList> NetworkInterface::queryAll(int family)
{
	...
	for (int j = 0; j < 2; ++j)
	{
		...
		// send request
		{
			...
			struct nlmsghdr *msg = (struct nlmsghdr *)fkit::malloc(msgLen);
			if (!msg) FKIT_SYSTEM_EXCEPTION;
		...
		// process reply
		{
			...
			if (bufSize < 0) FKIT_SYSTEM_EXCEPTION;

			void *buf = fkit::malloc(bufSize);
			if (!buf) FKIT_SYSTEM_EXCEPTION;
...
Ref<NetworkInterfaceList> NetworkInterface::queryAllIoctl(int family)
{
...

	{
		...
		ifc.ifc_len = capa;
		ifc.ifc_req = (struct ifreq *)fkit::malloc(capa);

		while (true) {
			if (::ioctl(fd, SIOCGIFCONF, &ifc) == -1)
				FKIT_SYSTEM_EXCEPTION;
			if (ifc.ifc_len == capa) {
				fkit::free(ifc.ifc_req);
				capa *= 2;
				ifc.ifc_len = capa;
				ifc.ifc_req = (struct ifreq *)fkit::malloc(capa);
				continue;
