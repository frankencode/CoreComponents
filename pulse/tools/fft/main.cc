...
int main(int argc, char *argv[])
{
    ...
    Thread fftWorker {
        [&tap, &plot]{
            ...
            auto buffer = CircularBuffer<int16_t>::allocate(n); // FIXME: establish consistent allocation notation for CircularBuffer
            for (Bytes data; tap.read(&data);) {
                for (long j = 0, m = data.itemCount<int16_t>(); j < m; ++j) {
                    ...
                    if (buffer.fill() == n) {
                        ...
                        for (long k = 0; k < n; ++k) {
                            v[k] = buffer.at(k); // FIXME: add index operator for CircularBuffer
                        }
                        for (long k = 0; k < l; ++k) { // FIXME: need a "dropFront()" method for CircularBuffer
                            buffer.popFront();
                        }
                        fft(&v);
