...
{
    char buf[CC_MEM_TRACE_BUF_SIZE];
    int fill = 0;
    while (size > 0) {
...
{
    char message[CC_MEM_TRACE_BUF_SIZE];
    char *eoi = message;
...
CC_MEM_UNUSED static void trace_inspect_size(const char *source, int line, const char *name, size_t size)
{
...
CC_MEM_UNUSED static void *trace_inspect_ptr(const char *source, int line, const char *name, void *ptr)
{
    char message[2 * CC_MEM_TRACE_BUF_SIZE];
    char *eoi = message;
    eoi = trace_text(source, eoi);
