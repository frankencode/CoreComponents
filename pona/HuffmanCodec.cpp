...
#ifdef PONA_HUFFMANCODEC_DEBUG_STATISTICS
	for (int i = 0; i < diversity; ++i)
	{
		PONA_DEBUG << i << ": H(" << codeTable_[i].value << ") = " << codeTable_[i].count << endl;
	}
#endif
                          ...
                          BitSource* source )
{
	...
	if (rawFill > rawCapacity)
		PONA_THROW(StreamSemanticException, "Decoding buffer of insufficient size provided.");
