...
class TestKeyExpansion: public TestCase
{
    ...
    void run() {
        FLUX_VERIFY(testKeyExpansion128());
        FLUX_VERIFY(testKeyExpansion196());
        FLUX_VERIFY(testKeyExpansion256());
    }
};
...
{
    FLUX_TESTSUITE_ADD(TestKeyExpansion);
    FLUX_TESTSUITE_ADD(AesExamples);
